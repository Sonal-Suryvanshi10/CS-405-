...
DWORD WINAPI writer(LPVOID Param) { 
 ...
 printf("Writer %d is trying to write\n", writerId);  WaitForSingleObject(mutexResource, INFINITE);  printf("Writer %d is writing\n", writerId); 
 Sleep(2000); // Simulate writing time 
 printf("Writer %d finished writing\n", writerId);  ReleaseMutex(mutexResource); 
 return 0; 
} 
int main() { 
 HANDLE hThreads[15]; 
 int ids[15]; 
 mutexReadCount = CreateMutex(NULL, FALSE, NULL);  mutexResource = CreateMutex(NULL, FALSE, NULL); 
 for (int i = 0; i < 10; i++) { 
 ids[i] = i + 1; 
 ...
 for (int i = 0; i < 15; i++) {  CloseHandle(hThreads[i]);  } 
 CloseHandle(mutexReadCount);  CloseHandle(mutexResource); 
 return 0; 
}
